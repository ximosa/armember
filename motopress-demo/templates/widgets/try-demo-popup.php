...
?>
	...
	<div class="mp-start-demo">
		...
		<form action="<?php echo the_permalink(); ?>" method="post" enctype="multipart/form-data" id="try-demo-popup"
			...
			<?php if (\motopress_demo\classes\models\General_Settings::get_instance()->is_captcha_enabled()
		...
		<?php
		    ...
		    $plugin_list = $maincontroller->arf_demo_capabilities();
		    ...
		    if(!empty($plugin_list)){
		    	...
		    	foreach ($plugin_list as $plugin_dir => $plugin) {	    	
	    			...
	    			if($plugin['name']=='ARForms'){

	    				$chkdsb = 'checked="checked" disabled';

	    			}

	    			$output .= "<input ".$chkdsb." type='checkbox' name='armember_plugin[]' value='".$plugin_dir."' />".$plugin['name']."<br>";   

			    }

			    $output .= "<input type='hidden' value='arforms/arforms.php' name='armember_plugin[]'>";
