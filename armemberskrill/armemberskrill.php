...
class ARM_Skrill{
    ...
    function __construct() {
        ...
        add_action('arm_payment_gateway_validation_from_setup', array(&$this, 'arm2_skrill_payment_gateway_form_submit_action'), 10, 4);
    ...
    function arm_skrill_load_textdomain() {
        load_plugin_textdomain('ARM_SKRILL', false, dirname(plugin_basename(__FILE__)) . '/languages/');
    }
    ...
    function upgrade_data_skrill() {
        ...
        if (version_compare($armnew_skrill_version, '1.0', '<')) {
    ...
    */
    ...
    function arm_add_skrill_payment_gateways($default_payment_gateways) {
        if ($this->is_version_compatible()) {
            ...
            $default_payment_gateways['skrill']['gateway_name'] = __('Skrill', 'ARM_SKRILL');
    ...
    function arm_skrill_admin_notices() {
        ...
        if($pagenow == 'plugins.php' || (isset($_REQUEST['page']) && in_array($_REQUEST['page'], (array) $arm_slugs))){
            if (!$this->is_armember_support())
                echo "<div class='updated updated_notices'><p>" . __('Skrill For ARMember plugin requires ARMember Plugin installed and active.', 'ARM_SKRILL') . "</p></div>";

            else if (!$this->is_version_compatible())
                echo "<div class='updated updated_notices'><p>" . __('Skrill For ARMember plugin requires ARMember plugin installed with version 3.0 or higher.', 'ARM_SKRILL') . "</p></div>";
        }
    }
    ...
    function arm_change_payment_gateway_tooltip_func($titleTooltip, $gateway_name, $gateway_options) {
        if ($gateway_name == 'skrill') {
            return __("You can find Merchant Email in your Skrill merchant account.", 'ARM_SKRILL');
            //return __("You can find Secret key and Public key in your Skrill account. To get more details, Please refer this", 'ARM_SKRILL')." <a href='https://dashboard.skrill.com/#/settings/developer' target='_blank'>".__("document", 'ARM_SKRILL')."</a>.";
        }
        return $titleTooltip;
    }
    ...
    function arm_gateway_callback_info_func($apiCallbackUrlInfo, $gateway_name, $gateway_options) {
        if ($gateway_name == 'skrill') {
            ...
            $apiCallbackUrlInfo = __('Please make sure you have set following callback URL in your skrill account.', 'ARM_SKRILL');
            $callbackTooltip = __('To get more information about how to set callback URL in your skrill account, please refer this', 'ARM_SKRILL').' <a href="'. ARM_SKRILL_DOC_URL .'" target="_blank">'.__('document', 'ARM_SKRILL').'</a>';
            //$apiCallbackUrlInfo = '<a href="'. ARM_SKRILL_DOC_URL .'" target="_blank">'.__('ARMember Skrill Documentation', 'ARM_SKRILL').'</a>';
    ...
    function arm_filter_gateway_names_func($pgname) {
        $pgname['skrill'] = __('Skrill', 'ARM_SKRILL');
        return $pgname;
    }
    ...
    function arm_skrill_recurring_trial($notice) {
        ...
        if ($this->is_version_compatible()){
            $notice .= "<span style='margin-bottom:10px;'><b>". __('Skrill (if Skrill payment gateway is enabled)','ARM_SKRILL')."</b><br/>";
            $notice .= "<ol style='margin-left:30px;'>";
            $notice .= "<li>".__('Skrill payment gateway does not support Trial period and future recurring date for Auto-Debit Payment method.','ARM_SKRILL')."</li>";
            $notice .= "</ol>";
            $notice .= "</span>";
        } 
    ...
    function arm_payment_related_common_message($common_messages) {
        if ($this->is_version_compatible()) {
            ?>
            <tr class="form-field">
                <th class="arm-form-table-label"><label for="arm_payment_fail_skrill"><?php _e('Payment Fail (Skrill)', 'ARM_SKRILL'); ?></th>
    ...
    */
    ...
    function arm_after_payment_gateway_listing_section_func($gateway_name, $gateway_options) {
        ...
        if ($gateway_name == 'skrill') { ?>
            <!-- <tr class="form-field arm_skrill_fields">
                <th class="arm-form-table-label"><?php //_e('Merchant ID', 'ARM_SKRILL'); ?> *</th>
                <td class="arm-form-table-content">
            ...
            <tr class="form-field arm_skrill_fields">
                <th class="arm-form-table-label"><?php _e('Merchant Account (email)', 'ARM_SKRILL'); ?> *</th>
                <td class="arm-form-table-content">
                    <input type="text" class="arm_active_payment_<?php echo strtolower($gateway_name); ?>" id="arm_skrill_merchant_email" name="payment_gateway_settings[skrill][skrill_merchant_email]" value="<?php echo (!empty($gateway_options['skrill_merchant_email'])) ? $gateway_options['skrill_merchant_email'] : ''; ?>" data-msg-required="<?php _e('Merchant Email can not be left blank.', 'ARMember');?>"/>
                </td>
            </tr>
        <?php
        }
    ...
    function arm2_skrill_payment_gateway_form_submit_action($payment_gateway, $payment_gateway_options, $posted_data, $entry_id = 0) {
        ...
        if ($payment_gateway == 'skrill' && isset($all_payment_gateways['skrill']) && !empty($all_payment_gateways['skrill'])) 
        {
            ...
            if(!empty($entry_data) && $current_payment_gateway == $payment_gateway)
            {
		    	...
		    	if($plan->has_trial_period() && $allow_trial)
	                    {
	                        $skrill_err_msg = '<div class="arm_error_msg"><ul><li>' . __('Skrill doesnot support Free trial/plan amount with auto recurring billing cycle.', ARM_skrill_TEXTDOMAIN) . '</li></ul></div>';
                ...
                $arm_skrill_payment_form_data['detail1_text']        = !empty($plan->description) ? strip_tags($plan->description) : '';
    ...
    function arm2_add_user_and_transaction($entry_id = 0, $skrill_response, $arm_display_log = 1) {
        ...
        if (isset($entry_id) && $entry_id != '') {
            ...
            if (isset($all_payment_gateways['skrill']) && !empty($all_payment_gateways['skrill'])) {
                ...
                if(is_array($skrill_response))
                {
                    $arm_skrill_subs_id = (!empty($skrill_response['customer_id'])) ? $skrill_response['customer_id'] : '';

                    $arm_skrill_trans_id = (!empty($skrill_response['transaction_id'])) ? $skrill_response['transaction_id'] : '';
                }
                else
                {
                    $arm_skrill_subs_id = (!empty($skrill_response->customer_id)) ? $skrill_response->customer_id : '';
                ...
                $extraParam['subs_id'] = $arm_skrill_subs_id;
                $extraParam['trans_id'] = $arm_skrill_trans_id;
    ...
    function arm_cancel_subscription_instant($user_id, $plan, $cancel_plan_action, $planData)
    {
        ...
        if(strtolower($user_payment_gateway) == 'skrill' && $user_selected_payment_mode == "auto_debit_subscription" && $cancel_plan_action == "on_expire" && $paly_cycle_data['rec_time'] == 'infinite')
        {
            $payment_log_table = $ARMember->tbl_arm_payment_log;
            ...
            $ARMember->arm_write_response(date("Y-m-d")." Instant Subscription cancel Transaction Data => ".json_encode($arm_cancel_subscription_instant));

            $skrill_transaction_id = $transaction->arm_transaction_id;

            $this->arm_cancel_subscription_immediately($skrill_transaction_id);
        }
    }


    function arm_cancel_subscription_immediately($arm_skrill_transaction_id)
    {
        global $ARMember, $arm_subscription_cancel_msg, $arm_global_settings;
        try
        {
            $arm_skrill_subscription_cancel_url = "https://www.skrill.com/app/query.pl?action=cancel_rec&trn_id=".$arm_skrill_transaction_id;
                                    
            $arm_skrill_subscription_cancel_request = wp_remote_get($arm_skrill_subscription_cancel_url);

            $ARMember->arm_write_response("Skrll Cancel Request Data => ".json_encode($arm_skrill_subscription_cancel_url));

            $arm_subscription_cancel_msg = '';

            return $arm_skrill_subscription_cancel_request;
        }
        catch(Exception $e)
        {
            $ARMember->arm_write_response("Error in Cancel Payment at Skrill Gateway Side => ".json_encode($e));

            $common_messages = $arm_global_settings->arm_get_all_common_message_settings();
            $arm_pg_sub_failed_msg = !empty($common_messages['arm_payment_gateway_subscription_failed_error_msg']) ? $common_messages['arm_payment_gateway_subscription_failed_error_msg'] : '';
            if(!empty($arm_pg_sub_failed_msg))
            {
                $arm_subscription_cancel_msg = $arm_pg_sub_failed_msg;
            }
            else
            {
                $arm_subscription_cancel_msg = __("Membership plan couldn't cancel. Please contact the site administrator.", 'ARM_SKRILL');
            }
        }
    }
                    ...
                    if (!empty($transaction)) {
                        ...
                        if ($payment_type == 'skrill') {
                             if($user_selected_payment_mode == 'auto_debit_subscription') {
                                ...
                                $ARMember->arm_write_response(date("Y-m-d")." Cancel Method Transaction Data => ".json_encode($transaction));

                                $ARMember->arm_write_response(date("Y-m-d")." Cancel Method Transaction ID => ".json_encode($skrill_transaction_id));

                                if (!empty($skrill_transaction_id) && !empty($arm_skrill_subscription_code))
                                {
                                    $arm_skrill_cancel_request = $this->arm_cancel_subscription_immediately($skrill_transaction_id);

                                    $ARMember->arm_write_response(date("Y-m-d")." Skrill Cancel Request Data => ".json_encode($arm_skrill_cancel_request));
...
function armskrill_plugin_api_call($def, $action, $args) {
    ...
    if (is_wp_error($request)) {
        $res = new WP_Error('plugins_api_failed', __('An Unexpected HTTP Error occurred during the API request.</p> <p><a href="?" onclick="document.location.reload(); return false;">Try again</a>', 'ARM_SKRILL'), $request->get_error_message());
    } else {
        $res = unserialize($request['body']);

        if ($res === false)
            $res = new WP_Error('plugins_api_failed', __('An unknown error occurred', 'ARM_SKRILL'), $request['body']);
    }
