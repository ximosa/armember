...
// <![CDATA[
    ...
    function arm_load_com_review_grid_after_filtered(msg) {
        jQuery('#armember_datatable').dataTable().fnDestroy();
        arm_load_com_review_grid(true, msg);
    }
    function arm_load_com_review_grid(is_filtered, msg) {
        var search_term = jQuery("#arm_com_review_search").val().trim();
        var com_review_search_term = (typeof search_term !== 'undefined' && search_term !== '') ? search_term : '';

        var start_data = jQuery("#arm_filter_pstart_date").val().trim();
        var com_review_search_start_data = (typeof start_data !== 'undefined' && start_data !== '') ? start_data : '';

        var end_data = jQuery("#arm_filter_pend_date").val().trim();
        var com_review_search_end_data = (typeof end_data !== 'undefined' && end_data !== '') ? end_data : '';
        ...
        var dt_obj = {
            "oLanguage": {
                ...
                "sEmptyTable": "No any review found.",
                "sZeroRecords": "No matching review found."
            },
            "bProcessing": false,
            "bServerSide": true,
            ...
            "fnServerParams": function (aoData) {
                aoData.push({'name': 'action', 'value': 'arm_com_review_list'});
                aoData.push({'name': 'com_review_search_term', 'value': com_review_search_term});
                aoData.push({'name': 'com_review_search_start_data', 'value': com_review_search_start_data});
                aoData.push({'name': 'com_review_search_end_data', 'value': com_review_search_end_data});
        ...
        jQuery('#arm_com_review_search').bind('keydown', function (e) {
            e.stopPropagation();
            if (e.keyCode == 13) {
    ...
    jQuery(document).ready(function () {
        arm_load_com_review_grid(false);
    });
			...
			<dl class="arm_selectbox">
				...
				<dd>
					<ul data-id="arm_manage_bulk_action1">
						...
						<li data-label="<?php _e('Approve', ARM_COMMUNITY_TEXTDOMAIN); ?>" data-value="approve_review"><?php _e('Approve', ARM_COMMUNITY_TEXTDOMAIN); ?></li>

						<li data-label="<?php _e('Disapprove', ARM_COMMUNITY_TEXTDOMAIN); ?>" data-value="disapprove_review"><?php _e('Disapprove', ARM_COMMUNITY_TEXTDOMAIN); ?></li>
...
<div class="arm_members_list">
	...
	<form method="POST" id="arm_com_review_list_form" class="data_grid_list" enctype="multipart/form-data" onsubmit="return arm_com_review_list_form_bulk_action();">
		...
		<div id="armmainformnewlist" class="arm_filter_grid_list_container">
            ...
            <div class="arm_datatable_filters arm_com_post_filter">
                <div class="arm_dt_filter_block arm_datatable_searchbox">
                    <label><input type="text" placeholder="<?php _e('Search', ARM_COMMUNITY_TEXTDOMAIN); ?>" id="arm_com_review_search" value="<?php echo $filter_search; ?>" tabindex="-1"></label>
                ...
                <div class="arm_dt_filter_block arm_dt_filter_submit arm_payment_history_filter_submit">
                    <input type="button" class="armemailaddbtn" id="arm_payment_grid_filter_btn" value="<?php _e('Filter', ARM_COMMUNITY_TEXTDOMAIN); ?>" onClick="arm_load_com_review_grid_after_filtered()"/>
                </div>
            </div>
            <div class="armclear"></div>
			...
			<input type="hidden" name="entries_grid" id="entries_grid" value="<?php _e('User Review', ARM_COMMUNITY_TEXTDOMAIN); ?>"/>
			...
			<input type="hidden" name="no_match_record_grid" id="no_match_record_grid" value="<?php _e('No matching review found.', ARM_COMMUNITY_TEXTDOMAIN); ?>"/>
			<input type="hidden" name="no_record_grid" id="no_record_grid" value="<?php _e('No any review found.', ARM_COMMUNITY_TEXTDOMAIN); ?>"/>
			...
			<input type="hidden" name="arm_action_type" id="arm_action_type" value=""/>
