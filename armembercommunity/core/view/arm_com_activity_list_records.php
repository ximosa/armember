...
// <![CDATA[
    ...
    jQuery(document).ready(function () {
        arm_load_com_activity_grid(false);
    });
    function arm_load_com_activity_grid_after_filtered(msg) {
        jQuery('#armember_datatable').dataTable().fnDestroy();
        arm_load_com_activity_grid(true, msg);
    }
    function arm_load_com_activity_grid(is_filtered, msg) {
        var search_term = jQuery("#arm_com_activity_search").val().trim();
        var com_activity_search_term = (typeof search_term !== 'undefined' && search_term !== '') ? search_term : '';

        var start_data = jQuery("#arm_filter_pstart_date").val().trim();
        var com_activity_search_start_data = (typeof start_data !== 'undefined' && start_data !== '') ? start_data : '';

        var end_data = jQuery("#arm_filter_pend_date").val().trim();
        var com_activity_search_end_data = (typeof end_data !== 'undefined' && end_data !== '') ? end_data : '';
        ...
        var dt_obj = {
            "oLanguage": {
                ...
                "sEmptyTable": "No any activity found.",
                "sZeroRecords": "No matching activity found."
            },
            "bProcessing": false,
            "bServerSide": true,
            ...
            "fnServerParams": function (aoData) {
                aoData.push({'name': 'action', 'value': 'arm_com_activity_list'});
                aoData.push({'name': 'com_activity_search_term', 'value': com_activity_search_term});
                aoData.push({'name': 'com_activity_search_start_data', 'value': com_activity_search_start_data});
                aoData.push({'name': 'com_activity_search_end_data', 'value': com_activity_search_end_data});
        ...
        jQuery('#arm_com_activity_search').bind('keydown', function (e) {
            e.stopPropagation();
            if (e.keyCode == 13) {
...
<div class="arm_members_list">
    ...
    <form method="POST" id="arm_com_activity_list_form" class="data_grid_list" enctype="multipart/form-data" onsubmit="return arm_com_activity_list_form_bulk_action();" >
        ...
        <div id="armmainformnewlist" class="arm_filter_grid_list_container">
            ...
            <div class="arm_datatable_filters arm_com_post_filter">
                <div class="arm_dt_filter_block arm_datatable_searchbox">
                    <label><input type="text" placeholder="<?php _e('Search', ARM_COMMUNITY_TEXTDOMAIN); ?>" id="arm_com_activity_search" value="<?php echo $filter_search; ?>" tabindex="-1"></label>
                ...
                <div class="arm_dt_filter_block arm_dt_filter_submit arm_payment_history_filter_submit">
                    <input type="button" class="armemailaddbtn" id="arm_payment_grid_filter_btn" value="<?php _e('Filter', ARM_COMMUNITY_TEXTDOMAIN); ?>" onClick="arm_load_com_activity_grid_after_filtered()"/>
                </div>
            </div>
            <div class="armclear"></div>
            ...
            <input type="hidden" name="entries_grid" id="entries_grid" value="<?php _e('User Activity', ARM_COMMUNITY_TEXTDOMAIN); ?>"/>
            ...
            <input type="hidden" name="no_match_record_grid" id="no_match_record_grid" value="<?php _e('No matching Activity found.', ARM_COMMUNITY_TEXTDOMAIN); ?>"/>
            <input type="hidden" name="no_record_grid" id="no_record_grid" value="<?php _e('No any Activity found.', ARM_COMMUNITY_TEXTDOMAIN); ?>"/>
