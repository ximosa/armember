...
class arm_membership_change_password_element_shortcode extends Widget_Base
{
    ...
    {
        return esc_html('ARMember Change Password','ARMemeber').'<style>
        .arm_element_icon{
    ...
    protected function register_controls()
    {
        ...
        $this->start_controls_section(
			'section_content',
			[
				'label' => esc_html__( 'ARMember Change Password', 'ARMember' ),
			]
		);
        $this->add_control(
			'title',
		...
		$forms = $wpdb->get_results("SELECT * FROM `" . $ARMember->tbl_arm_forms . "` WHERE `arm_form_type`='change_password' ORDER BY `arm_form_id` ASC", ARRAY_A);
		$default = $cnt = 0;
		if(!empty($forms)){
